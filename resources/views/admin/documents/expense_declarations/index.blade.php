...
@section('title', 'Declaraciones Juradas de Gastos')

@section('styles')
    <style>
        .expense-card,
        .expense-card .card-body,
        .expense-card .table-responsive,
        .expense-card .dropdown,
        .expense-card td,
        .expense-card th {
        ...
        <div class="d-flex justify-content-between align-items-center my-4">
            <div>
                <h1 class="h3 mb-0 text-gray-800 fw-bold">
                    <i class="fas fa-file-invoice-dollar text-danger me-2"></i>Declaraciones Juradas de Gastos
                </h1>
                <p class="text-muted small mb-0">Sustento formal de gastos sin comprobante según Ley N° 28411 y Directiva de Tesorería</p>
            </div>
            <div>
                @can('expense_declarations.create')
                    <a href="{{ route('expense-declarations.create') }}" class="btn btn-primary shadow-sm">
                        <i class="fas fa-plus me-1"></i> Nueva Declaración Jurada
                    </a>
                @endcan
            </div>
        </div>
        ...
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-body">
                <div class="row g-3 align-items-end">
                    <div class="col-md-4">
                        <label class="form-label small fw-semibold text-muted">Buscar por N°, Servidor, DNI o Conceptos</label>
                        <div class="input-group">
                            <span class="input-group-text bg-light border-end-0"><i
                                    class="fas fa-search text-muted"></i></span>
                            <input type="text" id="filter_search" class="form-control border-start-0"
                                placeholder="Ej: DJG-000001, Juan Perez, 12345678, Supervisión...">
                        </div>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <label class="form-label small fw-semibold text-muted">Fecha de Declaración</label>
                ...
                <div class="table-responsive p-3">
                    <table class="table table-hover align-middle w-100" id="expenseTable">
                        <thead class="table-light">
                            <tr>
                                <th style="width: 120px;">N° Declaración</th>
    ...
    <script>
        $(document).ready(function() {
            const table = $('#expenseTable').DataTable({
                ...
                responsive: true,
                ajax: {
                    url: "{{ route('expense-declarations.get') }}",
                    data: function(d) {
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    ...
                    text: 'Esta acción cancelará la declaración jurada de gastos y su flujo de firmas.',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('expense-declarations.delete') }}",
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                id: id
                            ...
                            success: function(resp) {
                                Swal.fire('Anulado', resp.message ||
                                    'La declaración jurada fue anulada.', 'success');
                                table.draw(false);
                            },
                            error: function(err) {
                                Swal.fire('Error', err.responseJSON?.message ||
                                    'No se pudo anular la declaración jurada.', 'error');
                            }
                        });
                    }
                });
