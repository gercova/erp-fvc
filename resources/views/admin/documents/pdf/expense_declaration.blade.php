...
<head>
    <meta charset="UTF-8">
    <title>Declaración Jurada de Gastos N° {{ $declaration->correlativo }}</title>
    <style>
        @page {
            margin: 25px 30px;
        }
...
<body>
        ...
        <tr>
            ...
            <td class="header-text">
                ...
                <div class="doc-title-box">
                    <span class="doc-title">DECLARACIÓN JURADA DE GASTOS</span>
                    &nbsp;&mdash;&nbsp;
                    <span class="doc-number">N° {{ $declaration->correlativo }}</span>
                </div>
            </td>
        </tr>
    </table>
    ...
    <div class="statement-box">
        Yo, <strong>{{ $declaration->servidor_nombres }}</strong>, identificado(a) con D.N.I. N° <strong>{{ $declaration->dni }}</strong>, 
        en mi condición de <strong>{{ $declaration->cargo }}</strong>, 
        <strong>DECLARO BAJO JURAMENTO</strong> haber realizado los gastos que a continuación se detallan por concepto de: 
        <em>{{ $declaration->conceptos }}</em>, 
        los mismos que por razones de fuerza mayor o lejanía del lugar no cuentan con comprobantes de pago autorizados por SUNAT, 
        asumiendo la plena responsabilidad administrativa, civil y penal conforme a ley.
    </div>
    ...
    <table class="items-table">
        <thead>
            <tr>
                ...
                <th>DETALLE DE LA GESTIÓN / CONCEPTO DEL GASTO</th>
        ...
        <tbody>
            @foreach($declaration->items as $index => $item)
                <tr>
                    ...
                    <td style="text-align: center;">{{ $item->fecha ? $item->fecha->format('d/m/Y') : '' }}</td>
                    <td>{{ $item->detalle_gestion }}</td>
                    <td style="text-align: right; font-weight: bold;">{{ number_format($item->importe, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="total-box">
        TOTAL GASTO DECLARADO: <span style="color: #003366; font-size: 11.5px;">S/ {{ number_format($declaration->total, 2) }}</span>
        @if($declaration->total_letras)
            <div style="font-size: 9px; font-weight: normal; color: #555; margin-top: 2px;">
                SON: {{ $declaration->total_letras }}
            </div>
        @endif
    </div>

    <div style="text-align: right; margin-bottom: 15px; font-size: 9.5px;">
        {{ $declaration->lugar ?: 'Uchiza' }}, {{ $declaration->fecha ? $declaration->fecha->translatedFormat('d \d\e F \d\e Y') : date('d/m/Y') }}
    </div>

    <!-- Firmas según flujo institucional -->
    @php
        $approvalsMap = $declaration->approvals->keyBy('step_order');
        $step1 = $approvalsMap->get(1); // Solicitante
        $step2 = $approvalsMap->get(2); // Coordinador / Jefe Inmediato
        ...
        <tr>
            ...
            <td>
                <div class="sign-box">
                    <div style="height: 48px;">
                        <div class="sign-stamp">DECLARANTE</div>
                        <div class="sign-content"><strong>{{ $declaration->servidor_nombres }}</strong></div>
                        <div class="sign-content">DNI: {{ $declaration->dni }}</div>
                    </div>
                    <div class="sign-title">FIRMA DEL DECLARANTE</div>
                </div>
            </td>
            ...
            <td>
                <div class="sign-box">
                    ...
                    <div class="sign-title">V° B° COORD. P.E. / JEFE INMED.</div>
                </div>
            </td>
